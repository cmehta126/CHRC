...
TCSH_AFNI=afni.${SID}.${TASK}_NL.tcsh
PWD_TASK=${SID}.${TASK}_NL
...
if (( $? == 0 )); then
	echo $SID >> ~/scratch60/pnc/status/completed_nback_NL.txt
else
	echo $SID >> ~/scratch60/pnc/status/incomplete_nback_NL.txt
fi
